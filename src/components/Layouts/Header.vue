<template>
    <header
        class="fixed inset-x-0 top-0 shadow-md md:shadow-none"
    ...
    >
        <div class="relative">
            <div class="flex justify-between mx-auto max-w-screen-3xl h-13 md:h-19">
                <div class="flex pl-4">
                    <slot name="logo">
                        <g-link to="/" class="relative flex items-center w-32 xs:w-40 md:w-48">
                            <g-image src="../../images/logo-ds.png" alt="logo" class="w-auto pl-2"/>
                        </g-link>
                    </slot>
                </div>
                <div
                    class="absolute inset-x-0 z-20 items-center justify-center flex-grow mt-2 mx-2 xs:left-auto xs:w-full top-full md:flex md:mt-0 md:mx-0 md:relative md:top-auto xs:max-w-xs md:visible md:max-w-none md:items-stretch"
                ...
                >
                    ...
                    >
                        <template v-for="menuItem in menu">
                            <g-link
                                :key="menuItem.id"
                                class="relative flex px-6 py-4 text-xs transition-all duration-200 ease-in-out overflow-hidden md:py-0 md:overflow-visible md:mr-3 xl:mr-10 group lg:text-base focus:outline-none"
                                :class="[white ? 'cha-nav-link md:cha-nav-link-color' : 'hover:text-gray-900', {'text-gray-900 font-bold': activeMenu(menuItem)}]"
                            ...
                            >
                                <div class="leading-tight md:relative md:flex">
                                    ...
                                    <div
                                        class="absolute inset-y-0 left-0 w-1 my-1 transition-all duration-200 ease-in-out rounded-r opacity-0 bg-gradient-b-blue-green md:bg-gradient-r-blue-green md:transform md:-translate-y-1/2 md:rounded md:mt-0 md:top-full md:bottom-auto md:inset-x-0 md:h-1 group-hover:opacity-100 md:w-auto"
                ...
                <div class="flex items-stretch md:pr-3">
                    <div class="flex items-stretch justify-end">
                        <g-link
                            class="inline-flex self-center justify-center px-4 py-2 tracking-tighter text-xs font-bold leading-none transition-all duration-200 ease-in-out rounded-full md:px-5 md:py-3 md:text-sm"
                        ...
                        <div @click.prevent="toggle" tabindex="1" data-toggle-menu class="flex items-center focus:outline-none">
                            <div class="relative z-50 inline-block p-2 mx-4 cursor-pointer md:hidden focus:outline-none rounded-full" :class="{'bg-gradient-tr-blue-green': open}">
            ...
            <div class="absolute bottom-0 left-0 w-full h-3 -mt-px transform translate-y-full cha-header-shadow opacity-0 md:opacity-10"></div>
        </div>
    </header>
</template>
