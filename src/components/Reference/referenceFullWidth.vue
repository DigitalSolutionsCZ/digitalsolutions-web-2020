<template functional>
    ...
    >
        <div class="relative z-20 p-4 md:p-8 xl:pb-8 xl:px-16 xl:pt-16">
            <g-link
                :to="props.baseUrl"
                :class="props.reference.referenceMultipleImages.length > 0 ? 'w-18/24 md:w-16/24' : 'w-full'"
                class="block"
            >
            ...
            <span
                class="inline-block mb-4 text-sm text-green-500"
                v-if="parent.mapObject(props.reference, ['vyberKlienta', 0, 'title'])"
            >
                {{ parent.mapObject(props.reference, ['vyberKlienta', 0, 'title']) }}
            </span>
            <div class="w-full text-xs leading-relaxed text-gray-900 md:text-base"
                 :class="props.reference.referenceMultipleImages.length > 0 ? 'md:w-12/24 xl:w-14/24' : 'w-full'"
        ...
        <div
            ...
            <div
                class="relative z-10 self-end col-start-2 col-end-7 row-start-1 row-end-1 pointer-events-auto md:col-start-8 md:col-end-11 translate-z-0"
                v-if="props.reference.referenceMultipleImages.length > 0">
                <g-link
                    :to="props.baseUrl"
                    class="relative block transition-transform duration-200 ease-in-out transform outline-none aspect-ratio-square xl:translate-x-8 xl:translate-y-0 xl:scale-1 group-hover:translate-x-0 will-change-transform translate"
                >
                    <g-image
